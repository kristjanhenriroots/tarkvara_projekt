...
void findneighbours(int P1, int P2, int *neighbours, int *neighbourcount, short **cells, int length) {
	*neighbourcount = 0;
	if(P2 + 2 <= length && cells[P1][P2+2] == 0){//Checks all neigbours and puts 1 in array if there ia a neighbour in that position
		neighbours[1] = 1;
		(*neighbourcount)++;
	}
...
void generateTree(short **cells, short **cellsets, int length) {
...
void treemaze(int length, short **maze){
	int i;
	short **cellsets = calloc(length + 4, sizeof(short*));
	for(i = 0; i < length + 4; i++) {
		cellsets[i] = calloc(length + 4, sizeof(short));
	}
